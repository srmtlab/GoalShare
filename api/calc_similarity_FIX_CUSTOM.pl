...
my $SIM_THRESHOLD = 0.001; # threshold of the similarity for linking goals 

#old ones
#my $WEIGHT_BOW = 0.5;              # alpha: weight of BoW (Bag-of-Words)
#my $WEIGHT_BOT = 1 - $WEIGHT_BOW;  # beta: weight of BoT (Bag-of-Topics)
#my $DECAY_SUPER = 0.4;  # d_sup: decay ratio for super-goals
...
my $allGoals = 1;
my ($test_name, $super, $sub, $alpha, $beta, $SIM_THRESHOLD, $filename, $build_db, $goal_uri) = @ARGV;
my $graph_uri_expresult = "http://collab.open-opinion.org/exp_$test_name";
#my $filename = "subset.txt";

my $DECAY_SUPER = $super;  # d_sup: decay ratio for super-goals
my $DECAY_SUB = $sub;    # d_sub: decay ratio for sub-goals
my $WEIGHT_BOW = $alpha;              # alpha: weight of BoW (Bag-of-Words)
my $WEIGHT_BOT = $beta;  # beta: weight of BoT (Bag-of-Topics)
init_dbh();
init_idf();
init_topics();    
init_mecab();
...
else
{
	...
	close(IN);
	foreach $goal_uri (keys(%goalset_flag)) {
		print "nn $goal_uri\n";
	}
	foreach $goal_uri (keys(%goalset_flagd)) {
		print "dd $goal_uri\n";
	}
}
my $te = $WEIGHT_BOT + $WEIGHT_BOW + $DECAY_SUB + $DECAY_SUB; 
print "ok[$test_name][$DECAY_SUB][$DECAY_SUPER][$WEIGHT_BOT][$WEIGHT_BOW][$te]\n";
    ...
    if (! $build_db) {
		...
		# Temporary fix ==> Goals that are in Eng. or has empty text are all similar 1.
		if ($sim >= $SIM_THRESHOLD && $sim < 1) {
...
select ?title ?desc where {
  ...
  optional {
    ...
    $query = "DELETE FROM SelfBagOfFeatures_${test_name} WHERE goal_url='$goal_uri';";
    $sth = $dbh->prepare($query);
    $sth->execute();
    $sth->finish();

    $query = "INSERT INTO SelfBagOfFeatures_${test_name} (goal_url,feat) VALUES ('$goal_uri','$feat_selfbof');";
    $sth = $dbh->prepare($query);
    $sth->execute();
    $sth->finish();
    ...
    $query = "DELETE FROM ContextBagOfFeatures_${test_name} WHERE goal_url='$goal_uri';";
    $sth = $dbh->prepare($query);
    $sth->execute();
    $sth->finish();

    $query = "INSERT INTO ContextBagOfFeatures_${test_name} (goal_url,feat,norm) VALUES ('$goal_uri','$feat_contbof',$norm_contval);";
    $sth = $dbh->prepare($query);
    $sth->execute();
    $sth->finish();
    ...
    $query = "DELETE FROM BagOfFeatures_${test_name} WHERE goal_url='$goal_uri';";
    $sth = $dbh->prepare($query);
    $sth->execute();
    $sth->finish();

    $query = "INSERT INTO BagOfFeatures_${test_name} (goal_url,feat) VALUES ('$goal_uri','$feat_bof');";
    $sth = $dbh->prepare($query);
    $sth->execute();
    $sth->finish();
	...
	if ($val >= $FILTER_THRESHOLD) {
	    my $query = "SELECT goal_urls FROM FilterWithFeature_${test_name} WHERE id=$id;";
	    my $sth = $dbh->prepare($query);
	    $sth->execute();
	    my $result = $sth->fetchrow_hashref();
		...
		$goal_urls .= $goal_uri;
		my $query_repl = "REPLACE INTO FilterWithFeature_${test_name} VALUES ($id, '$goal_urls')";
...
sub init_dbh {
	...
	#Init tables TODO Delete existing
	
	
	my $query =  "CREATE TABLE IF NOT EXISTS BagOfFeatures_$test_name (
  goal_url varchar(240) NOT NULL,
  feat text,
  PRIMARY KEY (goal_url)
) ENGINE=InnoDB DEFAULT CHARSET=utf8";
  my $sth = $dbh->prepare($query);
  $sth->execute();
	
	$query =  "CREATE TABLE IF NOT EXISTS ContextBagOfFeatures_$test_name (
  goal_url varchar(240) NOT NULL,
  feat text,
  norm double DEFAULT NULL,
  PRIMARY KEY (goal_url)
) ENGINE=InnoDB DEFAULT CHARSET=utf8";
   $sth = $dbh->prepare($query);
    $sth->execute();
    
    $query =  "CREATE TABLE IF NOT EXISTS SelfBagOfFeatures_$test_name (
  goal_url varchar(240) NOT NULL,
  feat text,
  PRIMARY KEY (goal_url)
) ENGINE=InnoDB DEFAULT CHARSET=utf8";
   $sth = $dbh->prepare($query);
    ...
    $query =  "CREATE TABLE IF NOT EXISTS FilterWithFeature_$test_name (
  id int(11) NOT NULL,
  goal_urls text,
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8";
...
sub get_self_bof {
    my $goal_uri = $_[0];
    try {
	my $query = "SELECT feat FROM SelfBagOfFeatures_${test_name} WHERE goal_url='$goal_uri';";
	my $sth = $dbh->prepare($query);
	$sth->execute();
	my $result = $sth->fetchrow_hashref();
	$sth->finish();
...
sub get_context {
	...
	foreach $key (keys %ancestors) {
		$context{$key} = $ancestors{$key};
    }
    foreach $key (keys %decendants) {
		$context{$key} = $decendants{$key};
    }
    return %context;
...
sub get_decendant_goals {
	...
	foreach $subgoal (@subgoals) {
	    ...
	    print "Saved value $subgoal $goal2weight{$subgoal}\n";
...
sub get_similar_goals {
    my ($goal_uria, $ref_id2val, $ref_context) = @_;
    my %id2val = %$ref_id2val;
    my %context = %$ref_context;
    my $goal_uri = $goal_uria;
    utf8::decode($goal_uri);
    my $decoded = $goal_uri;
    print "Decode comparison[$goal_uri][$decoded]\n\n";
    $context{$decoded} = 1;
    ...
    my $query = "SELECT goal_urls FROM FilterWithFeature_${test_name} WHERE $condition;";
    my $sth = $dbh->prepare($query);
    $sth->execute();
	...
	foreach $goal (@goal_urls) {
	    #utf8::encode($goal);
	    #print "\n\n IS IN GOAL set [$goal][$goalset_flag]\n\n";
	    if ($goal2sim{$goal}) {
		next;
	    }
	    	
	    print "goal pair: $goal_uri, $goal\n";
	    print "    [context!!] $goal, $context{$goal}\n";
	    #print "!!!!!!!! candidate of simgoal: $goal\n";
	    if (!$context{$goal} && ( $allGoals || $goalset_flag{$goal} || $goalset_flagd{$goal} ) ) {
	    	# || $goalset_flagd{$goal} || $goalset_flage{$goal}) 
		print "NOT FILTERED OUT!!!!\n";

		my $query_bof = "SELECT feat FROM BagOfFeatures_${test_name} WHERE goal_url='$goal'";
