...
my $SIM_THRESHOLD = 0.3; # threshold of the similarity for linking goals 
...
if (! @ARGV) {
    print STDERR "Usage: perl calc_similarity_exp.pl GOALSET_FILE\n";
    exit(0);
}

my %goalset_flag = ();

my $goalset_file = $ARGV[0];
open(IN, "$goalset_file");
while(<IN>) {
    s/\s+$//;
    $goalset_flag{$_} = 1;
}
...
sub get_term2freq {
	...
	#print "$surface,$base,$mecab_feats[0]\n";
...
sub get_similar_goals {
	...
	print "Error in $goal_uri\n";
	print "SELECT goal_urls FROM FilterWithFreature WHERE $condition;\n";
};
    my $sum_topics = 0;
	...
	foreach $goal (@goal_urls) {
	    utf8::encode($goal);
	    if ($goal2sim{$goal} || ! $goalset_flag{$goal}) {
		next;
	    }
		...
		print("goal1: $goal_uri, goal2: $goal\n");
